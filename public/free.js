...
function start3MinCountdown(endTime) {
  ...
  const timer = setInterval(() => {
    ...
    if(timeLeft <= 0){
      ...
      const waitEndTime = Date.now() + 24*60*60*1000; // 24h
...
freeBtn.addEventListener("click", () => {
  const endTime = Date.now() + 3*60*1000; // 3 mins
  localStorage.setItem("freeEnd", endTime);
  start3MinCountdown(endTime);
});

// Restore timers automatically
...
// Only restore/start after login
