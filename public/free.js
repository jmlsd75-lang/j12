...
export function initPay(showToastFn) {
    ...
    function showPayInfoPage() {
        // Admin check — MUST declare userEmail before using it
    ...
    function enterPaidMode(startTime, totalSeconds) {
        ...
        // Force remove free mode overlay if active
        ...
        timerInterval = setInterval(() => {
            ...
            if (remaining <= 0) {
                ...
                // Re-hook pay button if it exists
    ...
    if (savedData) {
        try {
            ...
            if (totalSeconds - elapsed > 0) {
                ...
                return; // Don't set up observer — user is already in paid mode
            } else {
                localStorage.removeItem(STORAGE_KEY);
            }
        } catch {
    ...
    // free.js creates the payBtn dynamically when wait mode starts.
    // This observer hooks into it the moment it appears in the DOM.
